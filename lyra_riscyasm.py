...
def tohex(num, z):
    hex_num = hex(num)[2:]
    return "0" * (z - len(hex_num)) + hex_num
...
def main(fname):
    with open(fname, "r") as asmfile:
        lines = macro(asmfile.readlines()[:-1])

    with open(f"{fname}.bin", "w") as binfile, \
            open(f"{fname}.bin.txt", "w") as txtbinfile, \
            open(f"{fname}.rom.bin", "w") as romfile, \
            open(f"{fname}.lst", "w") as lstfile:
        ...
        for line in lines:
            ...
            if lineparts[0].lower() == "li":
                op1, op2 = lineparts[1], lineparts[2]

                if not ('#' in op2):
                    if op2[0:2] == "0x":
                        op2u = (int(op2[2:], 16) >> 4) & 15
                        op2l = int(op2[2:], 16) & 15
                    else:
                ...
                lineu = f"\tsliu {op1} {op2u}"
                linel = f"\tslil {op1} {op2l}"
